...
import Ride from '../../contracts/Ride.sol/Ride.json';
...
export default function RideShareSteps(props) {
  ...
  const [rideContractAddress, setRideContractAddress] =useState('0x1Fd9CceDA0B9eFD3e720441305C53a09B87Ff2d6');
  const [tempi,setTempi]=React.useState('')
  ...
  const handleNext = async (e) => {
    ...
    if (localStorage.getItem('type') !== null && localStorage.getItem('type') === "0") {
      ...
      if (activeStep === 0) {
        ...
        isLoading(true);
        updateSeats(value)
          console.log("activestate=1 start")
          rideManager.methods.requestRide(
            ...
            .once('receipt', async (receipt) => {
              ...
              console.log(data);
             const addr1=data[5][data[5].length - 1];
             console.log(addr1)
             setRideContractAddress(addr1)


              setActiveStep((prevActiveStep) => prevActiveStep + 1);
            });
                     let temp=[{name:"Raj",contact:"1234567890",carNo:"MH 12 AB 1234",rating:"4.5",amount:"100",ethAddress:"0xA82689dE3309BaA8A64A3AEcf1079e026660215B"},{name:"Shyam",contact:"1234590",carNo:"CH 12 AB 124",rating:"4.5",amount:"100",ethAddress:"0x5B38Da6a701c568545dCfcB03FcB874556beddC4"}]
                    
          const tempList = temp.map(data => {
             console.log(parseInt(data.name,16))
            return (
              [
               data.name,
                data.contact,
