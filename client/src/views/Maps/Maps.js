...
const Maps = compose(
  withProps({
    googleMapURL:
      "https://maps.googleapis.com/maps/api/js?key=AIzaSyC87fftpSPt2ttOpm3kvzwdfyOmZX9Mu9A&v=3.exp&libraries=geometry,drawing,places",
        ...
        getRoute: (source, destination) => {
          ...
          travelMode: google.maps.TravelMode.DRIVING
            ...
            (result, status) => {
              if (1) {
                console.log(result);
                localStorage.setItem('sourceLat', 19.1217707406339);
                localStorage.setItem('sourceLng', 72.83944134193857);
                localStorage.setItem('destinationLat', 19.17060002212831);
                localStorage.setItem('destinationLng', 72.79517092313263)
                localStorage.setItem('distance', 15.4);
